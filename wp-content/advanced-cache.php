...
if (false === $plugin_location) {
	...
	if (strcasecmp('https://sdlccorp.com/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-sdlccorp.com.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-sdlccorp.com.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-sdlccorp.com.php');
}
